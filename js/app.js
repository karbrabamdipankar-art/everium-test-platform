  ...
  async loadUsers() {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (!users.length) {
          tbody.innerHTML = '<tr><td colspan="5"><div class="empty-state"><h3>No users found</h3><p>Add students and teachers in SkillTrack Pro, then sync here.</p></div></td></tr>';
          return;
        }
          ...
          <tr>
            <td>
              <div style="display:flex;align-items:center;gap:.6rem">
                <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,var(--navy),var(--royal));display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:.8rem">${(u.name||u.username)[0].toUpperCase()}</div>
            ...
            <td>
              <div style="display:flex;gap:.4rem;flex-wrap:wrap">
                ...
                ${u.role==='student' ? `<button class="btn btn-sm btn-ghost" onclick="AdminDash.resetAttemptPrompt('${u.id}','${u.name}')">Reset Attempt</button>` : ''}
              </div>
            </td>
          </tr>`).join('');
      }
  ...
  async runSync() {
    ...
    try {
      ...
      if (res.success) {
        ...
        Toast.error('Sync error: ' + res.message);
      }
