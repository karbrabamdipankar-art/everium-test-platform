...
const AdminDash = {
  ...
  _renderUsersTable(allUsers) {
    ...
    const filtered = allUsers.filter(u => u.role === this._userTab);

    if (!filtered.length) {
      const label = this._userTab === 'student' ? 'students' : 'teachers';
      tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
        <h3>No ${label} found</h3>
        <p>Add ${label} in SkillTrack Pro, then sync here.</p>
      </div></td></tr>`;
      return;
    }
      ...
      <tr>
        <td>
          <div style="display:flex;align-items:center;gap:.6rem">
            <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,${u.role==='student'?'var(--royal),#3b6fd4':'var(--gold-dark),var(--gold)'});display:flex;align-items:center;justify-content:center;color:${u.role==='student'?'white':'var(--navy)'};font-weight:700;font-size:.8rem">${(u.name||u.username)[0].toUpperCase()}</div>
            <div><div style="font-weight:600">${u.name||'—'}</div><div style="font-size:.72rem;color:var(--text-muted)">${u.email||'—'}</div></div>
          </div>
        </td>
        ...
        <td>
          <div style="display:flex;gap:.4rem;flex-wrap:wrap">
            <button class="btn btn-sm btn-${u.active?'warning':'success'}" onclick="AdminDash.toggleStatus('${u.id}',${!u.active})">
              ${u.active?'Disable':'Enable'}
            </button>
            ${u.role==='student' ? `<button class="btn btn-sm btn-ghost" onclick="AdminDash.resetAttemptPrompt('${u.id}','${(u.name||'').replace(/'/g,"\\'")}')">Reset Attempt</button>` : ''}
          </div>
        </td>
      </tr>`).join('');
  },
  ...
  async runSync() {
    ...
    if (!url || url.includes('YOUR_SKILLTRACK')) { Toast.error('Set SKILLTRACK_API_URL in js/config.js first.'); return; }
    Loading.show('Syncing from SkillTrack Pro…');
    try {
      const res = await API.syncFromSkillTrack(url);
      Loading.hide();
      if (res.success) {
        Toast.success(`Sync complete! ${res.usersAdded} users added, ${res.coursesAdded} courses added.`);
        // Reload dependent panels so UI reflects new data immediately
        await this.loadSyncPanel();
        if (this.currentView === 'users') await this.loadUsers();
        if (this.currentView === 'course-mgmt') await CourseMgr.init();
        await this.loadOverview();
      } else {
        Toast.error('Sync error: ' + (res.message || 'Unknown error. Check that SKILLTRACK_API_URL is correct.'));
      }
    } catch(e) { Loading.hide(); Toast.error('Sync failed: ' + e.message); }
  },
